...
The repository is public, the G3 public bootstrap is closed, and all five G4 representative archetypes have passed their independent completion audit. G4 is closed and G5 progressive bounded migration is active. The first G5 tranche adds the complete reviewed Maebashi Witches V1 analysis tree and seven Character Index v2 discovery records. The second adds the complete compact Mass Effect comparative corpus and two reviewed Commander Shepard player-archetype records under the intentionally cross-root study binding. The third adds the bounded reviewed Genshin Impact Furina V1 analytical corpus and one Character Index v2 discovery record. The fourth adds 23 reviewed Cinderella Girls mobile-game analytical artifacts and nine Character Index v2 discovery records. Character Index v2 hardening is complete. The materialized candidates are the U149 tranche under `series/the-idolmaster-cinderella-girls-u149/`, one partial IDOLY PRIDE provenance-ledger slice under `series/idoly-pride/`, and one partial DJFW control-state slice under `studies/doujinshi-fanwork-comparative-taxonomy/`. The DJFW slice contains one corpus map plus 17 TSV worksheet projections and their structure manifest; it is not a complete study migration, and names of sibling artifacts inside the corpus map are not claims that those artifacts are present in Git. The P04 reference control records six hash-verified Gakuen Idolmaster ZIP identities as destination-free `REFERENCE_DRIVE` metadata. It adds no archive bytes, Gakuen Idolmaster analysis tree, or Drive URL to Git. The prior unapproved P05/YonaiP v1 tuple remains withdrawn and was not imported; the current YonaiP discovery record is an independently authored schema-v2 record.
