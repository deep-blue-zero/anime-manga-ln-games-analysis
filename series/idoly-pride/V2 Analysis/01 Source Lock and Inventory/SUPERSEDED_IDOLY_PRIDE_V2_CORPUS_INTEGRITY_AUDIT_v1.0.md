...
The source corpus is sufficiently complete, internally traceable, and technically coherent to begin Phase 1.
...
The project may proceed to Phase 1 with the following rule:

> Broad game-text coverage begins from `analysis_bundles`; exact evidentiary descent uses `idoly-ingest`; the anime and visual layers are invoked whenever form, delivery, or visual presentation matters; every known gap remains explicit rather than silently normalized.
