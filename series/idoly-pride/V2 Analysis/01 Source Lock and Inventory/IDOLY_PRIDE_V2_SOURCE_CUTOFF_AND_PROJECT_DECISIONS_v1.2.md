...
## 6. Manager identity, customization, and branch canon

Decision:

For V2 analysis, the customizable game manager **is treated as the continuing Makino Kouhei (牧野航平)** from the anime. This is a governing identity rule rather than an unresolved equivalence hypothesis. The game places the player in Makino's established Hoshimi role and preserves overwhelming continuity signals, including his narrative history, visual identity, and voiced presentation; editable naming therefore functions as player-facing interface parameterization rather than evidence for a replacement protagonist.

Only explicit primary-source contradiction may reopen the identity question. The mere presence of `{user}`, a custom name, or selectable dialogue does not constitute such a contradiction.

Player-choice branches remain part of the source corpus, but branch semantics must be preserved:

- fixed history / branch-invariant action or dialogue = ordinary Makino canon;
- mutually exclusive selectable lines = authored **Makino-compatible possibility space**, not multiple events that all literally occurred;
- recurring traits across independent branches may strengthen stable characterization;
- a trait supported only by one optional branch must retain that limitation in the claim ledger.

Phase 2 therefore maintains a **Makino Player-Branch Canon Ledger**, not a ledger whose purpose is to decide whether the player manager is Makino.
