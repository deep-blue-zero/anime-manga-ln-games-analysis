---
series: MHA
corpus: MHA_SP2
artifact_type: update_manifest
scope: V41
generation: V2
method_generation: V2.1
status: PROVISIONAL
proposed_status_if_promoted: canonical
source_boundary: Japanese manga Volume 41, narrative V41:p005-p195
do_not_use_as_current_authority: true
supersedes: []
superseded_by: []
---

# MHA SP2 — Volume 41 Update Manifest (Provisional Working Draft)

## Tranche state

**PROVISIONAL WORKING PACKET / NOT CANONICAL MHA AUTHORITY.**

This manifest records the proposed canonical consequences of the V41 second-pass reading. The analysis and pre-promotion semantic diffs are tracked in the V41 working packet, but the existing canonical cumulative ledgers and series current-state surface have not yet been mutated by this tranche. Presence of the packet in Git history or on `main` does not itself promote those proposed changes. The evidence Drive remains unchanged.

The packet was originally staged on the now-retired temporary branch `chatgpt/my-hero-academia-analysis`. Continuing MHA work is now routed through `series/my-hero-academia` under the repository's branch lifecycle policy.

## Source closure

- Canonical source: `My Hero Academia - Vol. 41 [Japanese].cbz`
- Drive ID: `1q6pj0t6mP59pOsywt8p58H3VWz2l52X_`
- Size: **104,293,245 bytes**
- SHA-256: `236c8ee8546dcc4c2007a9ffedced6e3489f1d0a2f721cea0d89b5dc75220700`
- Narrative start: `V41:p005`
- Narrative endpoint: **`V41:p195`**
- Chapters: **411-422**
- `p196+`: non-sequential end/publication/extra matter.
- V42 excluded.

## Governing analytical transition

**V40:** inheritance without possession.

**V41:** **agency without self-sufficiency**.

> Power, identity and relation can remain authored without remaining individually self-contained. Midoriya relinquishes OFA but retains the biography it formed; Shigaraki is deeply engineered without becoming wholly AFO-authored; Kurogiri/Shirakumo is treated as mixed rather than purely restored; Eri contributes within a limit while keeping an ordinary future; and the final attack is individually enacted but collectively enabled.

Supporting rules:

- **rescue without annexation**;
- **designed conditions are not total authorship**;
- **weakness can be relationally generative without suffering being intrinsically good**;
- **distributed heroism preserves differentiated roles and expertise**.

## Major state closures

### Resolved

1. `危機感知` cliffhanger: Shigaraki stole Danger Sense; Fourth separates; Shigaraki can use it.
2. OFA transfer plan: Midoriya/vestiges deliberately expend inherited factors through Shigaraki's interior wound.
3. Shimura-memory entry: Midoriya reaches child Tenko and protects him from Kotaro's strike.
4. Shigaraki adult agency claim: `僕が選んだ` / League hands / `敵の…ヒーロー` are explicit.
5. AFO internal return: occurs after Tenko's interior state is overwhelmed.
6. AFO developmental engineering: direct Shimura-family manipulation, latent-factor removal, engineered destruction-only Quirk copy, hatred cultivation for OFA theft.
7. Midoriya arm loss/recent Rewind: both arms lost and restored within Eri's short temporal window.
8. Kurogiri/Warp Gate reinforcement route reopens.
9. Aizawa explicitly rejects pure-restoration Kurogiri/Shirakumo model in favor of mixed/altered continuity.
10. Eri's horn contribution is self-chosen, bounded and future-oriented through desire to sing.
11. Sero explicitly rejects the thesis that tragedy is desirable/necessary for strength.
12. AFO's Yoichi attachment is affectively unique: Yoichi's loss drains color from his world.
13. Midoriya states OFA has been lost while `残り火` remain at the V41 boundary.
14. Aizawa explicitly states reciprocal need between Midoriya and the group.
15. Final V41 strike is enabled by distributed path-making/support.

### Revised / complicated

1. **Shigaraki formation:** AFO's causal responsibility is much larger than previously authorized; Shigaraki's later agency is not thereby zeroed out.
2. **Rescue:** child-Tenko recognition must coexist with adult-Tomura recognition.
3. **AFO:** total-authorship claim becomes a central possession/ownership tell rather than neutral omniscient narration.
4. **Kurogiri/Shirakumo:** continuity is mixed; “restore the true Shirakumo” is too binary.
5. **OFA:** factor possession, embodied learning and identity separate.
6. **Hero society:** plural participation is a network architecture, not anti-expertise occupational flattening.
7. **Strength/trauma:** recovery after tragedy is possible; tragedy is not narratively justified as necessary strength production.
8. **Midoriya:** final strength includes visible need / reciprocal dependence rather than maximal self-sufficiency.

### Still OPEN at V41 endpoint

- whether `V41:p195` definitively defeats AFO;
- final Shigaraki/Tenko/AFO identity-control state;
- survival/death and final custody status of Shigaraki/Tenko/AFO body;
- Midoriya's permanent OFA/Quirk state after embers;
- Kurogiri/Shirakumo final state;
- Eri's longer post-horn medical/Quirk state;
- final war medical/casualty accounting beyond what V41 directly confirms;
- postwar legal/institutional/social settlement;
- final Midoriya/Shigaraki rescue/accountability meaning.

## Character-model consequences

| Character | Entering readiness | V41 result | Rationale |
|---|---|---|---|
| Midoriya | `specialist_ready` | remains | major refinement: OFA relinquishment, rescue under refusal, help-reception, reciprocal dependence |
| Shigaraki | `specialist_ready` | remains | causal model revised by AFO engineering; adult League/choice evidence preserved |
| AFO | `strong` | remains, very strongly strengthened | design/manipulation/Yoichi attachment deepens core model; ordinary reciprocal breadth still missing |
| Aizawa | `specialist_ready` | remains | mixed-identity reasoning + distributed command + reciprocity |
| Present Mic | no forced top-tier change | no forced promotion | grief/identity relation strengthened, ordinary breadth still thin |
| Kurogiri | `moderate` | remains, very strongly strengthened | relational response + mixed continuity; independent ordinary desire/final identity open |
| Eri | `strong` | remains, very strongly strengthened | later autonomous bounded Rewind use + ordinary future desire; longer baseline still gates specialist tier |
| Sero | `insufficient/emerging` | `emerging (strongly grounded)` | explicit ordinary motive + anti-tragedy value + direct rescue action |
| Kota | `emerging` | remains, strengthened | longitudinal reciprocal participation impulse after earlier rescue |
| Mineta | `emerging` | remains, strengthened | continued prosocial self-risk under depletion; breadth still too narrow |
| Ojiro / Sato / Aoyama / Hagakure | existing tiers | remain | high-stakes contribution strengthens but does not create breadth sufficient for promotion |

## Proposed mutable homes

### New canonical files when authorized

1. `02 Sequential Readings/MHA_SP2_V41_DEEP_READING.md`
2. `08 Audits and Manifests/MHA_SP2_V41_UPDATE_MANIFEST.md`

### Existing canonical files with local diffs staged

1. Class 1-A Character State Ledger
2. U.A. Students/Staff Character State Ledger
3. Family/Civilian/Social Actor Ledger
4. Villain/Antagonist Character State Ledger
5. Relationship State Ledger
6. Character Model Readiness Index
7. Power Philosophy Ledger
8. Hero Society Ledger
9. Recognition and Failed Rescue Ledger
10. Villain Formation Ledger
11. Japanese Vocabulary and Voice Ledger
12. Visual Motif Ledger
13. Callback and Payoff Ledger
14. First-Pass Correction Ledger
15. Primary Source Locator
16. Source Inventory
17. Current State and Corpus Map

### Explicitly screened but not mutated

- **Pro Hero Character State Ledger:** V41 pro appearances are routed to Hero Society because they primarily bear on system participation rather than sufficient standalone character reconstruction.
- **Project-global corpus/routing indexes:** no series identity or canonical root changes; MHA's high-water mark belongs in the series-local current-state map.

## Publication/promotion gates

A future authorized promotion transaction must:

1. re-fetch every existing target and verify current blob SHA;
2. reconcile any moved base rather than applying stale fragments;
3. materialize the canonical V41 deep reading and manifest through the continuing `series/my-hero-academia` branch under then-current governance;
4. apply cumulative ledgers/index updates atomically with the V41 reading;
5. ensure all frontmatter source boundaries and top-level current-state snapshots agree;
6. run the applicable pre-publication checks or connector-mode equivalent safety controls;
7. use only an owner-authorized ordinary integration route into `main`, with remote-head drift rechecked immediately before integration;
8. require the exact resulting `main` commit to pass the repository audit before treating promotion as closed.

## Next sequential operation

**Volume 42**, starting from unresolved `V41:p195`, is the next prospective source operation **only after separate authorization to continue analysis** or as otherwise directed by the user.
