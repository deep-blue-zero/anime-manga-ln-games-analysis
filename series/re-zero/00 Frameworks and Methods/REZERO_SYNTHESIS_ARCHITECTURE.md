...
source_boundary: "Japanese main light novel V01-V43 admitted; V44-V45 known acquisition gaps; non-spine witnesses excluded until separately admitted"
...
5. `REZERO_SOURCE_LOCK_AND_INVENTORY.md` for the next exact source;
6. `REZERO_MASTER_LONGITUDINAL_LEDGER.md`;
7. the immediately prior frozen volume's claim ledger and outbound question horizon;
8. only the additional specialist or witness-specific artifact required for the operation.
...
- A dedicated locator index or evidence matrix is deferred until cross-volume retrieval becomes materially unreliable. If promoted, it becomes the canonical navigation layer, not a second claims ledger.
...
The project expects later character monographs and may earn relationship/ensemble syntheses, but neither is created from importance or popularity alone.
...
- contradictions, counterevidence, and falsifiers.
...
Readiness values are `NOT_READY`, `ACCUMULATING`, `CHECKPOINT_READY`, and `MONOGRAPH_READY` or `SPECIALIST_READY`. Advancement requires cited evidence and may be downgraded if later source shows the apparent pattern was state-bound. Local monographs do not authorize edits to `characters/registry.jsonl` or `CHARACTER_ANALYSIS_INDEX.md`.
...
full-series synthesis readiness review
        |
        v
full-series synthesis -> validation / release audit -> freeze
```

Claim/evidence stabilization precedes any synthesis that depends on the disputed claims. Character monographs may begin when their own threshold is met, but a final character model waits for the relevant sequential and relationship state. Adaptation work cannot become an input to main-LN literary conclusions without an explicit cross-witness comparison stage. A final full-series synthesis requires the complete-series source boundary or an expressly named bounded corpus; exhaustion of the currently admitted V43 range does not equal published-series completion.

## 13. Responsibility matrix

| Analytical dimension | Sequential observation | Cumulative canonical home | Mature/final destination | Initialization state at repair |
|---|---|---|---|---|
| Claims and revisions | material claims, counterevidence, six-state adjudication | master ledger claim tables | relevant specialists and full-series synthesis | `INITIALIZED_BACKFILLED_V01_V02` |
| Route/event-state | local state, trigger, outcome, durability, carry-forward | master ledger event-state tables | memory/continuity or mechanics specialist if earned | `INITIALIZED_BACKFILLED_V01_V02` |
| Knowledge/asymmetry | holder, source, confidence, nonholder, consequence | master ledger knowledge table | relationship, agency, and continuity syntheses | `INITIALIZED_BACKFILLED_V01_V02` |
| Relationships | each party's remembered history, trust, disclosure, dependency, power | master ledger relationship table | character monographs; dyadic/ensemble synthesis if earned | `INITIALIZED_BACKFILLED_V01_V02` |
| Character state/stress | goals, self-model, conditions, behavior, recovery, counterexample | master ledger character/readiness tables | character monographs/models | `INITIALIZED_BACKFILLED_V01_V02` |
| Institutions/factions/power | rule/doctrine, actor behavior, capacity, resources, status, coercion | master ledger institution table | institutional/political specialist if earned | `INITIALIZED_BACKFILLED_V01_V02` |
| Mechanics/world model | speaker theory, observation, cost, exception, corroboration | master ledger mechanics table | mechanics/metaphysics specialist if earned | `INITIALIZED_BACKFILLED_V01_V02` |
| Ordinary life/preferences | only model-diagnostic routines, tastes, labor, comfort, low-stakes conduct | master ledger ordinary-life table | character/relationship or ordinary-life specialist if earned | `INITIALIZED_BACKFILLED_V01_V02` |
| Japanese wording/register | wording-sensitive terms, address, titles, pragmatic shifts, exact locus | master ledger terminology table plus VNN evidence loci | language/register specialist; character and translation analysis | `INITIALIZED_BACKFILLED_V01_V02` |
| Prospective questions | bounded falsifiers/predictions before next volume | master ledger open-question register and prior VNN freeze | next VNN adjudication; checkpoints | `INITIALIZED_FROM_V02_OUTBOUND_HORIZON` |
| Source identity/locators | source ID, hash, locus convention | source lock plus VNN reading; ledger routes only | evidence/locator index if retrieval burden earns it | `SOURCE_LOCK_EXISTING`; dedicated index deferred |
| Character/relationship/specialist readiness | cited accumulation and threshold judgment | master ledger readiness tables | promoted canonical artifacts | `INITIALIZED`; none ready at V02 |
| Supplemental/IF/WN | no opening until admission and safe horizon | source lock + witness protocol; later witness-specific reading | bounded cross-witness specialist | `NO_WITNESS_ADMITTED` |
...
|---|---|---|
| architecture repair and architecture role-gap audit | `PREMIUM_QUALITY_FIRST` | default because errors propagate through the full project |
