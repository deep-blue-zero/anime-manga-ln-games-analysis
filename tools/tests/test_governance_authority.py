...
class PublicGovernanceInvariantTests(unittest.TestCase):
    ...
    def test_series_registry_is_unique_and_routes_fail_closed(self) -> None:
        ...
        historical_ids = {
            row["analysis_subject_id"] for row in historical_cinderella_rows
        }
        self.assertEqual(
            [
                row
                for row in cinderella_rows
                if row["analysis_subject_id"] in historical_ids
            ],
            historical_cinderella_rows,
        )
        self.assertEqual(
            {
                row["analysis_subject_id"] for row in cinderella_rows
            }
            - historical_ids,
            {
                "the-idolmaster:hisakawa-hayate@cinderella-girls-mobile-games",
                "the-idolmaster:jougasaki-mika@cinderella-girls-mobile-games",
                "the-idolmaster:jougasaki-rika@cinderella-girls-mobile-games",
            },
        )
    ...
    def test_g6_all_materialized_representations_have_three_ledger_legs(self) -> None:
        ...
        self.assertEqual(len(mappings), 2939)
        self.assertEqual(set(mappings), set(results))
        self.assertEqual(set(mappings), set(plans))
        for representation_id, mapping in mappings.items():
            with self.subTest(representation_id=representation_id):
