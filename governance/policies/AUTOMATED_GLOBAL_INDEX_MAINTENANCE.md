...
1. On stable analytical push events, `.github/workflows/repository-audit.yml` validates authored content against an in-memory projection of exactly the five routing outputs. All source, character, link, schema, authority, identity, and publication checks remain applicable. Every successful stable-branch source audit publishes a pending `Repository integration audit` status until housekeeping and the full final audit complete. If projected routing differs from committed bytes, the validator additionally reports `AWAITING_SYNCHRONIZATION`. It does not certify integration readiness or write files.
...
5. If no output changed and the source audit passed, it rechecks both remote heads and dispatches the full exact-commit audit for the unchanged source SHA, then exits without a commit. This certifies the branch boundary and current-`main` ancestry before a final success status can be published.
...
Queued runs superseded by a newer branch head exit successfully with a notice and no publication. If the branch or `main` advances before publication, preserve the newer work and reconcile/revalidate the final head; a superseded no-op is not validation of that newer commit. Use ordinary non-forced pushes and inspect ambiguous push results before retrying. An analytical branch behind current `main` must incorporate it before housekeeping can finalize the branch.
