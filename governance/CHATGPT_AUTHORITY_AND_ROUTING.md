...
The GitHub repository is public and the complete approved G5 text boundary is materialized, but Google Drive remains the analytical authority. Git is a nonauthoritative candidate until a separately approved and verified G8 activation. Route discovery inside the candidate through `MANGA_ANIME_CORPUS_INDEX.md`, `series/registry.json`, and `CHARACTER_ANALYSIS_INDEX.md`; do not treat public visibility or materialization completeness as authority activation.

The three crosswalk ledgers bind every migrated representation to its Drive identity, revision, source hash, transformation, and exact Git bytes. Reference-only binaries, primary-source media, generated extraction corpora, native authoring artifacts, and empty/excluded records have no Git payload destination. Migration operations may not rewrite, reorganize, delete, or clean Drive without separate authorization.
