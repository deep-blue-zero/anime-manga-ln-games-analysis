...
Never add raw acquisition evidence, source media, private migration evidence, credentials, unreviewed binaries, symlinks, submodules, or hooks. The approved workflows are the read-only `.github/workflows/repository-audit.yml` and the default-branch, exact-output-allowlisted `.github/workflows/global-index-housekeeping.yml`; every other workflow remains prohibited without a separately reviewed owner amendment. The housekeeping boundary and declarative inputs are defined by `governance/policies/AUTOMATED_GLOBAL_INDEX_MAINTENANCE.md`. Recurring tranches follow `governance/policies/RECURRING_TRANCHE_PROCESS.md`: one owner-approved package may cover the bounded transformation-to-non-forced-push lifecycle, while its exact path manifest remains the staging allowlist. Do not use `git add .` or wildcard staging. A correction stays inside that approval only when it is expressly allowed, mechanical, non-semantic, confined to named paths, and followed by every affected local check plus the final GitHub audit.
...
Concurrent title work follows `governance/policies/BRANCH_LIFECYCLE.md`. Use the stable `series/<stable-slug>` branch for continuing series analysis and `studies/<stable-slug>` for continuing study work. On those branches, author analysis and any required `.repository/` declarative inputs; do not manually write the automation-owned global outputs. Temporary bootstrap, repair, migration, and automation branches must be pruned after their exact tips are verified as preserved on a green `main`; never delete an unverified or unmerged branch.
